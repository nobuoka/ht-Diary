...
=head2 OAuth CONFIGURATION

本アプリケーションは, ユーザー認証のためにはてな OAuth を使用します. 

=head3 Hatena OAuth CONF FILE

はてな OAuth の認証に用いるための client credentials の設定は, 
config/oauth_hatena.conf ファイルに記述します. 形式は JSON 形式です. 

config/oauth_hatena.conf.template が雛形ですので, 
これを参考に記述してください.
...
=head2 PROXY SERVER CONFIGURATION

はてなの OAuth 認証を行う際に, はてなのサーバーと通信を行います. 
その際, プロキシサーバーを通しての通信が必要な場合は, プロキシサーバーのアドレスを
